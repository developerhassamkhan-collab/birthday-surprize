...
import { getRomanticAudio, playRomanticMusic } from "@/lib/useRomanticMusic";
...
export function LoadingScreen({ done }: { done: boolean }) {
  ...
  const handleEnter = () => {
    ...
    // Pre-load audio during direct user gesture
    const audio = getRomanticAudio();
    if (audio && typeof audio.load === "function") {
      audio.load();
    }

    // Start playing after exactly 2 seconds delay
    window.setTimeout(() => {
